...
export const Logo: React.FC<LogoProps> = ({ className = '', showTagline = false }) => {
  return (
    <Link 
      href="/" 
      className={`group inline-flex items-center gap-2.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#06b6d4] rounded-md p-1 transition-all ${className}`}
      aria-label="o11y.works Home"
    >
      {/* Telemetry Signal Node Icon */}
      <div className="relative flex items-center justify-center w-8 h-8 rounded-lg bg-surface border border-white/10 group-hover:border-[#06b6d4]/40 transition-colors p-1 overflow-hidden">
